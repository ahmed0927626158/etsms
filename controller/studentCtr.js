...
const getStudent = (req, res) => {
    ...
    //console.log(id)
    try {
        const selectStudents = "SELECT s.id,s.firstname, s.middlename,s.lastname,s.gender,s.letter_grade_id,DATE_FORMAT(s.birthdate, '%Y-%m-%d') AS birthdate,s.address,g.grade,l.letter FROM student AS s JOIN letter_grades AS l ON l.id=s.letter_grade_id JOIN grade AS g ON l.grade_id=g.id WHERE s.company_id=? AND g.company_id=? AND l.company_id=? AND s.id=?"
        dbPool.query(selectStudents, [company_id, company_id, company_id,id], (error, result) => {
            if (error) {
